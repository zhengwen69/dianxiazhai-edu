...
**摘要**：人工智能教育应用正从单点工具向协同化Agent体系演进，而面向"学"的Agent设计尚乏系统关注。本文以《固体废弃物污染防治技术》研究生课程9-Agent协同体系为例，阐述Agent角色从"教"到"伴学"的双向映射方法。教师端9个Agent覆盖教学生产全周期，学生端同一组Agent重定义为学伴团队。研究提出角色重塑三原则（镜像而非简化、服务对象翻转、角色自主重定义），建立双向映射表，以Agent-S和Agent-B为案例展示角色转化深度，并与四种AI教育系统多维对比。小规模试用（5名研究生/2周）提供了初步定性反馈。技术实现基于提示词工程与RAG。
...
人工智能在教育领域的应用正在经历从"单点工具"到"协同体系"的范式跃迁[1,2]。早期的AI教育工具——智能批改、自动组卷、知识图谱生成——以独立功能模块形式嵌入教学流程，每个工具解决一项特定教学任务。这种"工具箱"模式虽然降低了使用门槛，但也造成了三个结构性缺陷。第一，碎片化。教师需要使用多个独立工具完成一门课程的全流程教学支撑，不同工具之间缺乏信息共享和流程衔接。第二，教中心化。当前AI教育工具的产品逻辑主要面向教师端需求——辅助备课、出题、批改——而面向学生端"如何学"的Agent设计尚未得到系统性关注[3,4]。第三，角色单薄。每个AI工具被赋予单一的功能角色（如"出题""批改""推荐"），而教学实践真正需要的是一组各司其职又相互协作的"角色"——就像一支教学团队不只有"讲课者"，还有"课程设计者""作业批改者""学情追踪者""案例打磨者"。

Agent（智能体）技术突破了这些局限。与单点工具不同，Agent是具有特定角色、明确职责和自主决策能力的智能实体。多个Agent可以组成一个协同工作体系，按照预设的流程和责任分工完成复杂的教学任务。这一理念在教育领域的应用尚处于早期探索阶段，但已经在课程开发[5]、个性化学习[6]和智能辅导[7]等方向展示了初步潜力。自ChatGPT问世以来，大语言模型对教育的深刻影响引发了广泛讨论[8]，教育部亦相继出台教育信息化2.0[9]与教育新基建[10]等政策，为AI教育应用提供了制度框架[11]。
...
![图1：教师端11-Agent协同教学体系工作流 / Fig.1 Teacher-side 11-Agent collaborative workflow](截图素材-SCREENSHOTS/SC07-Agent工作流.png)，出具A/B/C/D四级评分报告，交叉校验大纲→讲稿→PPT→试卷的一致性。

### 1.3 技术架构与提示词设计

9-Agent体系的技术实现基于大语言模型（LLM）的提示词工程（Prompt Engineering），辅以检索增强生成（Retrieval-Augmented Generation, RAG）支撑课程知识库的精准问答[12,13]。各Agent的技术实现不依赖独立的模型训练或微调——每个Agent的本质是一组精心设计的系统提示词（System Prompt）和任务模板[14]，运行于统一的LLM后端（当前支持GPT-4o与Claude 3.5 Sonnet双模型切换）。

Agent的提示词设计遵循"角色定义→知识领域→任务模板→输出规范"四层结构。以Agent-S（故事化案例拆解）为例，其系统提示词的结构如下：（1）角色定义——"你是一位教育叙事分析师，专长于从纪实文学中提取具有教学价值的叙事片段"；（2）知识领域——绑定课程主讲教师七部纪实小说的全文文本（约200万字，经向量化后存储于RAG知识库）、五段叙事拆解法（背景→冲突→决策→结局→启示）的方法论文档、错配-修复框架的七子框架定义；（3）任务模板——"给定一个教学周次主题（如'空间错配'），从小说库中检索与该主题最相关的1-3个叙事片段（800-1200字），对每个片段执行五段拆解，生成3道分层讨论题（基础理解/框架应用/批判反思）"；（4）输出规范——选段须标注S编号（S001-S021）、拆解须标注方法论语块、讨论题须标注对应OBE课程目标编号。

Agent之间的协同通过两种机制实现。其一，Agent-0（教务长·质量总控）作为审核节点，接收所有Agent的产出，执行格式规范性、OBE合规性和知识准确性的自动校验，出具通过/退回/修正三级反馈，退回的产出携带具体修改指令返回源Agent重新生成。其二，Agent之间通过共享知识库实现"底层统一"——所有Agent读取同一套课程框架文档（叙·框·境·创教学法定义、错配-修复框架七子框架、八周教学设计方案、OBE课程目标矩阵），确保各Agent产出的教学内容在术语、框架和逻辑上保持一致[15]。

RAG知识库是本体系的技术基石。课程专属知识库包含三类文档：教学理论文档（教学法定义、框架体系、OBE矩阵）、素材文档（七部小说全文、S001-S021选段标注、场景脚本库）和教学产出文档（教学大纲、讲稿、作业题库、试卷模板）。知识库采用分段向量化存储（Embedding），Agent在生成任务时通过语义检索调用相关文档片段，有效降低了LLM在领域特定知识上的幻觉风险[16,17]。
...
![图3：教师端Agent-S故事化案例拆解卡片 / Fig.3 Teacher-side Agent-S card](截图素材-SCREENSHOTS/SC08-Agent-S卡片.png)
...
![图2：学生端学习流6步 / Fig.2 Student-side 6-step learning flow](截图素材-SCREENSHOTS/SC09-学习流6步.png)9个学伴Agent在这6步中各司其职。
...
就核心差异而言，9-Agent体系与通用AI工具最本质的区别不在于"功能多少"，而在于两个结构性的设计选择。其一，角色分工而非功能堆叠——9个Agent各有专属职责，学生面对的是"有明确能力边界的学伴团队"，而非"一个什么都会的工具"。这种设计避免了通用AI助手常见的"能力模糊"问题——学生不知道AI在什么任务上擅长、应该在什么情境下信任它[18]。其二，双向映射而非单向服务——同一Agent编号在教师端和学生端承担不同的角色，但共享同一套课程知识体系。这一设计的理论意义在于：它提供了一种"教学生产力"与"学习支持力"共享底层认知框架的协同方式，而非在"教师工具"和"学生工具"之间建立两套独立系统[15]。
...
[1] HOLMES W, TUOMI I. State of the Art and Practice in AI in Education
[2] 黄荣怀, 刘德建, 刘晓琳, 等. 人工智能在教育中的应用：机遇与挑战
[3] 祝智庭, 胡姣. 教育数字化转型的本质探析与研究展望
[4] 刘三女牙, 杨宗凯, 李卿. 人工智能赋能教育变革：观察与思考
[5] WOOLF B P. Building Intelligent Interactive Tutors: Student-Centered Strategies for Revolutionizing E-Learning
[6] RUSSELL S, NORVIG P. Artificial Intelligence: A Modern Approach
[7] LUCKIN R, HOLMES W, GRIFFITHS M, et al. Intelligence Unleashed: An Argument for AI in Education
[8] LO C K. What Is the Impact of ChatGPT on Education? A Rapid Review of the Literature
[9] 教育部. 教育信息化2.0行动计划
[10] 教育部. 关于推进教育新型基础设施建设构建高质量教育支撑体系的指导意见
[11] 杨宗凯. 以教育数字化引领教育现代化
[12] 余胜泉. 人工智能赋能教育变革：技术框架与未来趋势
[13] 吴飞, 杨洋, 何晓飞. 大语言模型在教育中的应用与反思
[14] MOLLICK E, MOLLICK L. Assigning AI: Seven Approaches for Students with Prompts
[15] 郑永和, 王一岩, 杨淑豪. 智能时代的人机协同教学：发展脉络、核心议题与趋势展望
[16] KASNECI E, SESSLER K, KUCHEMANN S, et al. ChatGPT for Good? On Opportunities and Challenges of Large Language Models for Education
[17] ZHAI X. ChatGPT User Experience: Implications for Education
[18] 钟秉林, 尚俊杰, 王建华, 等. ChatGPT对教育的挑战与应对
...
\*通讯作者：黄正文（1969—），男，博士，教授，硕士生导师，研究方向：资源与环境普惠教育。ORCID: https://orcid.org/0009-0006-8724-9432。Email: huangzhengwen@cdu.edu.cn

**English Authors**: WANG Hongbin¹, XIE Zeyu², HUANG Zhengwen¹\*
...
\*Corresponding author: HUANG Zhengwen (1969—), Ph.D., Professor. ORCID: https://orcid.org/0009-0006-8724-9432. Email: huangzhengwen@cdu.edu.cn

**作者简介**：
- 王鸿斌（1986—），男，博士，副教授，研究方向：人工智能教育应用
- 谢泽宇（1991—），男，博士研究生（在读），研究方向：工程教育
- 黄正文（1969—），男，博士，教授，硕士生导师，研究方向：资源与环境普惠教育。ORCID: 0009-0006-8724-9432
...
| 主办 | 清华大学 |
| 刊期 | 月刊 |
| ISSN/CN | ISSN 1009-8097，CN 11-4525/N |
| 影响因子 | 复合 IF 4.67 |
| 投稿平台 | https://xjjs.cbpt.cnki.net/ |
| 编辑部邮箱 | xdjyjs@tsinghua.edu.cn |
| 字数要求 | 8000-10000 字（含图表、参考文献、英文摘要） |
| 摘要 | 中文 200-300 字 + 英文对应 |
| 关键词 | 3-8 个，中英对照 |
| 参考文献格式 | GB/T 7714-2015 顺序编码制，正文上标标注 |
| 作者信息 | 投稿时另页提交（正文匿名盲审） |
| 图表规范 | 图题/表题须中英双语，随文嵌入 |
| 标题层级 | 一 →（一）→ 1 →（1）→ ① |
| 页面设置 | A4，上2.3/下2.3/左1.8/右1.7cm，双栏 |
| 审稿制度 | 双向匿名审稿，约 3 个月 |
| 审稿标准 | "六有"：有价值 / 有创新 / 有观点 / 有综述 / 有研究 / 有规范 |
| 版面费 | 录用后按通知缴纳 |
| 栏目 | 教学研究 / 技术应用与开发（P3 适合前者） |
| 2025 重点选题 | AI + 教育应用、多智能体系统、教育数字化转型 |
