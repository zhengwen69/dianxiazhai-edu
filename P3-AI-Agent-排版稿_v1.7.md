...
本研究的技术贡献在于提出了一种“提示词工程+RAG知识库”的低成本Agent实现路径——不依赖模型微调，仅通过系统提示词的角色化设计和课程专属知识库的语义检索，即可构建具有领域专业性的Agent协同体系[17]。初步试用反馈表明，Agent-S和Agent-B在案例解读和论文框架指导方面获得了试用者的正面评价，但跨会话记忆能力的缺失是当前最突出的技术局限。
